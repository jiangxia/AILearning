...
### 自动内容分析
- 判断层级（Pattern/Protocol/Practice/Products/Laws）
- Laws层判断标准：涉及法律法规、权利义务、案例分析、维权策略等法律相关内容
- 识别适合的文档类型
- 提取核心知识点
...
#### 通用格式（Pattern/Protocol/Practice/Products层）
```markdown
# [层级] | [吸引标题]
...
- **Laws层**：法律分析、案例解读、维权实战（如：劳动法分析、合同纠纷、维权指南）
...
Protocol | 对话设计的底层逻辑，让AI真正理解你.md
Practice | Prompt工程实战：从入门到精通.md
...
**注意：Laws层标题格式特殊，必须使用 "Laws AI时代 | " 开头**
...
#### 1. 文件位置强制检查
- ✅ 根据层级判断（Pattern/Protocol/Practice/Products/Laws），将文章保存到正确的content子目录
- ✅ 不得保存到项目根目录
- ✅ 文件保存后立即验证位置是否正确
...
#### 2. 格式强制检查
- ✅ 文章标题必须使用格式：`[层级] | [吸引标题]`
- ✅ **Laws层标题格式特殊**：必须使用 `Laws AI时代 | [吸引标题]`
- ✅ 文件名必须与文章标题完全一致
- ✅ Laws层文章必须包含法律声明（位置：## 深入交流之后，关于本人之前）
- ✅ 必须严格按照PSO结构执行
- ✅ 必须包含完整的固定结尾内容
